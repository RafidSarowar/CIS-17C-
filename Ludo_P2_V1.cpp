 ...
 * Purpose: CIS 17C Project 2 - Version 1
 *
 ...
 *
 * Project 2 goal:
 * Extend the Project 1 final version with the new monthly concepts:
 *   - Recursion
 *   - Recursive sorting
 *   - Hashing
 *   - Trees
 ...
 *
 * Version 1 goal:
 *   Start small. Take the Project 1 final version (Ludo_STL_V4_Final.cpp)
 *   and add the FIRST new concept: a binary search tree (BST).
 *
 * New concepts shown in this version:
 *   1. A MoveNode binary search tree that stores every Move from the game,
 *      keyed by the dice roll value. Equal rolls are pushed into a list
 *      stored at that node so the tree stays a true BST on the roll key.
 *   2. Recursive functions:
 *        - insertMove()      recursively inserts a Move into the BST
 *        - inOrderPrint()    recursive in-order traversal (sorted by roll)
 *        - countNodes()      recursive node counter
 *        - treeHeight()      recursive height calculation
 *        - destroyTree()     recursive cleanup of the BST
 *
 * Everything else (list, map, set, queue, stack, deque, iterators, STL
 * algorithms, no vector) is kept the same as the Project 1 final version.
 */
...
int main()
{
    ...
    ofstream logFile("ludo_p2_v1_log.txt");
    if (!logFile)
    {
        cout << "Error: could not open ludo_p2_v1_log.txt\n";
        return 1;
    }
    ...
    cout << "\nVersion 1 (Project 2) complete: added a recursive BST keyed on roll value.\n";
    cout << "Results were saved to ludo_p2_v1_log.txt\n";
...
{
    cout << "==================================================\n";
    cout << "     CIS 17C PROJECT 2: TREES, GRAPHS - VERSION 1\n";
    cout << "==================================================\n";
...
void writeLogHeader(ofstream &logFile, const map<string, Player> &players, const set<int> &safeTiles)
{
    logFile << "CIS 17C Project 2 - Version 1 Log\n";
    logFile << "Players: ";
...
void saveLeaderboard(const deque<ScoreRow> &leaderboard)
{
    ofstream outFile("ludo_p2_v1_leaderboard.txt", ios::app);
