 ...
 * Purpose: CIS 17C Project 2 - Version 2
 *
 * Project 2 background:
 * Project 1 finished as an STL-based Ludo game using list, map, set, queue,
 * stack, deque, iterators, and STL algorithms (no vector).
 *
 * Version 1 added the first new Project 2 concept: a binary search tree
 * (BST) of moves keyed on dice roll value, with several recursive helper
 * functions (insertMove, inOrderPrint, countNodes, treeHeight, destroyTree).
 *
 * Version 2 improvements over Version 1:
 *   1. Recursive merge sort is now used to sort the leaderboard instead of
 *      std::sort. mergeSortLeaderboard() and mergeLeaderboard() are written
 *      from scratch using recursion (divide and conquer).
 *   2. A recursive power function, powerRecursive(), grants a "double roll"
 *      bonus: when a player rolls double sixes back to back across two
 *      turns is rare, so instead the bonus is based on consecutive sixes -
 *      the bonus amount is 2 raised to the consecutiveSixes power, computed
 *      recursively.
 *   3. A recursive digit sum function, digitSumRecursive(), is used to
 *      compute a fun "lucky number" stat for each player based on their
 *      total moves.
 *   4. A recursive stack-history printer, printHistoryRecursive(), prints
 *      the move history stack using recursion instead of a while loop.
 *
 * The Version 1 binary search tree (MoveNode, insertMove, inOrderPrint,
 * countNodes, treeHeight, destroyTree) is kept unchanged.
 */
...
int main()
{
    ...
    ofstream logFile("ludo_p2_v2_log.txt");
    if (!logFile)
    {
        cout << "Error: could not open ludo_p2_v2_log.txt\n";
        return 1;
    }
    ...
    cout << "\nVersion 2 (Project 2) complete: added recursive merge sort,\n";
    cout << "recursive power/digit-sum helpers, and recursive stack printing.\n";
    cout << "Results were saved to ludo_p2_v2_log.txt\n";
...
{
    cout << "==================================================\n";
    cout << "     CIS 17C PROJECT 2: TREES, GRAPHS - VERSION 2\n";
    cout << "==================================================\n";
...
void writeLogHeader(ofstream &logFile, const map<string, Player> &players, const set<int> &safeTiles)
{
    logFile << "CIS 17C Project 2 - Version 2 Log\n";
    logFile << "Players: ";
...
void saveLeaderboard(const deque<ScoreRow> &leaderboard)
{
    ofstream outFile("ludo_p2_v2_leaderboard.txt", ios::app);
